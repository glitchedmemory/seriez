...
export async function GET(req: NextRequest) {
  ...
  // Use the poster_url already stored in DB — no per-request external refetch.
  // Posters are saved at track time (AniList for anime, TMDB for movie/tv), so
  // the library should serve them directly for instant loading.
  const items = data.map((t: any) => {
    const poster = t.poster_url || null;
    return {
      tmdbId: t.tmdb_id,
