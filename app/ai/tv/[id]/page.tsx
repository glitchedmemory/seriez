...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return {
    title: `TV Show Rating — Seriez Score`,
    description: `Seriez community rating and details for this TV show. Track movies, TV shows, and anime with Seriez.`,
    openGraph: {
      title: `Seriez Score — TV Show #${id}`,
      description: `Community rating from Seriez — the all-in-one tracking platform for movies, TV shows, and anime.`,
    },
  };
}

export default async function AITVPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  ...
  try {
    const res = await fetch(
      `${TMDB_BASE}/tv/${tvId}?api_key=${API_KEY}&language=en-US`,
  ...
  const rating = Math.round((show.vote_average || 0) * 10) / 10;
    ...
    <div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 700, margin: "0 auto", padding: "3rem 2rem", background: "#0a0a0a", color: "#e5e5e5", minHeight: "100vh" }}>
      ...
      <article style={{ background: "#111", borderRadius: 16, padding: "2rem", border: "1px solid #1a1a1a" }}>
        <div style={{ display: "flex", gap: "1.5rem", alignItems: "flex-start", flexWrap: "wrap" }}>
          {posterUrl && (
            <img src={posterUrl} alt={show.name} width="150" height="225" style={{ borderRadius: 12, objectFit: "cover", flexShrink: 0 }} />
          )}
          <div style={{ flex: 1, minWidth: 250 }}>
            ...
            {show.tagline && <p style={{ color: "#999", fontStyle: "italic", margin: "0 0 1rem 0" }}>{show.tagline}</p>}

            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap" }}>
              <span style={{ background: "#6366f1", color: "#fff", padding: "4px 14px", borderRadius: 8, fontSize: "1.3rem", fontWeight: 800 }}>
                Seriez Score: {rating}/10
              </span>
              <span style={{ color: "#999", fontSize: "0.9rem" }}>
                Based on {show.vote_count?.toLocaleString() || 0} community ratings
              </span>
            </div>
            ...
            {(show.networks || []).length > 0 && (
              <p style={{ color: "#777", fontSize: "0.85rem", margin: "0 0 1rem 0" }}>
                Network: {show.networks.map((n: any) => n.name).join(", ")}
              </p>
            )}
            ...
            <div style={{ marginTop: "1.5rem", display: "flex", gap: "1rem", flexWrap: "wrap" }}>
              ...
              <a href="https://seriez.app"
                style={{ background: "#1a1a1a", color: "#6366f1", padding: "12px 24px", borderRadius: 10, textDecoration: "none", fontWeight: 600, fontSize: "1rem", display: "inline-block" }}>
