...
export function SearchClient({
  ...
  const [trendingSearches, setTrendingSearches] = useState<TrendingItem[]>(initialTrending);
  ...
  // Fetch trending searches on mount only if not provided by server
  useEffect(() => {
      ...
      <div className="px-4">
        {!query && !selectedYear && (
          <>
            ...
            {trendingSearches.length > 0 && (
              <PublishedCollections initialData={initialCollections.length > 0 ? initialCollections : undefined} />
            )}
          </>
        )}
