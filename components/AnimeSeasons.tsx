...
export default function AnimeSeasons({
  ...
  // Split into direct seasons (title overlap >= 2) and related (overlap < 2)
        ...
        <div className="flex flex-wrap gap-2">
          {related.map((r) => (
            <a
              key={r.id}
              href={`/title/${r.id}?type=anime`}
              className="px-4 py-2 rounded-lg text-xs font-medium transition-all bg-bg-card text-text-secondary border border-border hover:bg-accent/10 hover:text-text-primary hover:border-accent/30"
            >
              {r.title}
            </a>
          ))}
